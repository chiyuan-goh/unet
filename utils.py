...
def binary_activation(tensor):
    cond = tf.less(tensor, tf.constant(0.5, tf.shape(tensor)))
    out = tf.where(cond, tf.zeros(tf.shape(tensor)), tf.ones(tf.shape(tensor)))
    return out


class MaskImageTensorBoard(TensorBoard):
    def __init__(self, image_every_x_epochs, output_name, log_dir, **kwargs):
    ...
    def set_model(self, model):
        ...
        if self.histogram_freq and self.merged is None:
            ...
            output_ = self.model.get_layer(self.output_name).output 
            mask = binary_activation(output) #should be (?, h, w, 1)
            



            tf.summary.image("hello", w_img) #####?????
